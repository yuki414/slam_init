...
class Pose2D():
    ...
    def calRelativePose(self, _npose, _bpose, _relPose):
        ...
        # 並進
        self.dx = _npose.tx - _bpose.tx
        self.dy = _npose.ty - _bpose.ty
        _relPose.tx = self.R0[0][0]*self.dx + self.R0[1][0]*self.dy
        _relPose.ty = self.R0[0][1]*self.dx + self.R0[1][1]*self.dy

        # 回転
        self.th = _npose.th - _bpose.th
        if (self.th< -180):
            self.th = self.th + 360
        elif (self.th >= 180):
            self.th = self.th - 360
        _relPose.th = self.th # !!:relはポインタが指定されてるのでこれをしても意味がないのでは
        _relPose.calRmat()
